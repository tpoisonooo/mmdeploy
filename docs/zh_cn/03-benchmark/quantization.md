...
|                                                                model                                                                |   dataset   | fp32 top-1 (%) | int8 top-1 (%) |
| :---------------------------------------------------------------------------------------------------------------------------------: | :---------: | :------------: | :------------: |
| [ViT-B32](https://github.com/open-mmlab/mmclassification/blob/master/configs/vision_transformer/vit-base-p32_ft-64xb64_in1k-384.py) | ImageNet-1k |    83.55\*     |    82.80\*     |

ncnn 尚未加速 ViT，CPU 推理实在太慢。只能用 imagenet 的小部分（2000/50000）做测试。

fp32 精度来自 ncnn fp32 推理；int8 版本量化了 mha/conv/gemm 三类 opr，同时启用 bias 校准。
